...
流程：先定位记录位置并自检（见下「使用前提」）→ 读 `SPEC.md`、`任务计划.md` 与在途变更 → 认领并写进度 → 再动代码；开发过程中把任务变更与功能变动写入对应记录；完成、暂停或交接前更新进度、写验证证据与 handoff。用户未提及本 skill 时同样必须执行，不得跳过记录直接改代码。
...
- **本地通道**（项目根在本地文件系统上，含代码仓与普通目录）：`git rev-parse --show-toplevel` 确认仓库根（若在 git 仓库内），否则用当前工作目录；直接读写，无连接问题；
- **Vault 通道**（项目根在 Obsidian Vault 内，或需要按项目名在 Vault 里定位项目）：先解析 Vault 根（**不得猜路径**），再探测连通性。
...
| 2 | 技能目录下的 `config.json`（与 `SKILL.md` 同级） | 本机配置，字段 `vaultRoot`；公开仓库只放 `config.example.json` 占位，真实路径不入库 |
...
- **所需通道不可用** → 立即停止流程，并**明确通知用户**（这是 skill 故障，不是普通报错）：
  ...
  > ⚠️ obsidian-project-management skill 无法读写本次记录（位置 …），本次无法记录。

  附排查清单并逐项给结论：① 记录目录是否存在、是否可写；② 项目根在 Vault 内时：Obsidian 是否在运行、MCP Connector 插件是否启用（Vault 的 `.obsidian/plugins/`）、端口 / token 是否变化（对照本机 MCP 客户端配置与插件 `data.json`）、Vault 根路径是否变化（对照 `config.json` / `OBSIDIAN_VAULT_ROOT`）。然后让用户选择：修复后重试，或本次不记录继续开发（继续时必须在最终回复注明「本次未写入记录」）。**不得静默跳过、不得假装已记录**；
- **降级** → Vault 通道的两条子通道只通一条时继续，并在首次回复注明（例如「MCP 未连接，本次用文件系统直读直写」）；
- 任何写入失败都如实报告，不得声称成功。

### B. 定位记录

1. 确定**项目根**：优先当前工作目录（在 git 仓库内时用仓库根）；用户指定了项目或目录的，以用户为准；拿不准就问，别自己挑；
2. 读 `<项目根>/records/SPEC.md`、`任务计划.md` 与在途变更；`records/` 不存在时，按 `references/records-templates.md` 建最小骨架（先经用户确认）；
3. 项目根在 Vault 内、或需要按项目名在 Vault 里找项目时：解析 Vault 根（见 A），读根 `README.md` 与 `软件开发/总任务排期.md`，列出 `软件开发/` 下的项目目录，按当前工作目录、仓库名、用户提到的项目名**模糊匹配**（例：`E:\noval-agent-dev` ↔「Noval Agent（多 Agent 小说创作系统）」）；匹配不到就列出候选请用户确认；
4. 用 MCP 时路径为 Vault 相对路径（如 `软件开发/总任务排期.md`），用文件系统时为绝对路径。
