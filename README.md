...
以 Obsidian Vault 为**唯一项目记录**的软件开发协作 Skill：触发后先**自动探索 Vault**（定位项目、读 `SPEC.md` / `任务计划.md` / 目标任务文件）并做**连接自检**——无法连接 Vault（MCP 与文件系统均不可用）时必须明确告知用户 skill 失效，不得静默跳过；完成、暂停或交接前更新任务进度、验证证据并创建/更新 handoff；范围变化必须先写入 `SPEC.md`，不得静默偏离。

适用于需要跨 Agent 持续跟踪的软件开发任务（开发新项目、任务变更、功能变动、新增功能、写代码、改代码、修 bug、重构、测试、代码评审、计划与排期、暂停、交接、复盘）。其中任务变更、功能变动、新增功能、新项目、计划调整等场景**强制写入 Vault**（见 `SKILL.md` 的「写入触发对照」）。一次性问答、查资料、与仓库无关的临时脚本**不触发**，不要写入 vault。
